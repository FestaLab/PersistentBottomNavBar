...
class BottomNavStyle3 extends StatelessWidget {
  ...
  Widget _buildItem(final PersistentBottomNavBarItem item, final bool isSelected, final double? height) => navBarEssentials!.navBarHeight == 0
      ...
      : AnimatedContainer(
          width: 100,
          height: height! / 1.0,
          duration: navBarEssentials!.itemAnimationProperties?.duration ?? const Duration(milliseconds: 1000),
  ...
  Widget build(final BuildContext context) {
    ...
    final double itemWidth = (MediaQuery.of(context).size.width - ((navBarEssentials!.padding?.left ?? MediaQuery.of(context).size.width * 0.05) + (navBarEssentials!.padding?.right ?? MediaQuery.of(context).size.width * 0.05))) / navBarEssentials!.items!.length;
    return Container(
